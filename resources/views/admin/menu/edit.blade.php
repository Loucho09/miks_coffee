        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-lg sm:rounded-xl p-8 transition-colors duration-300">
                ...
                <form method="POST" action="{{ route('admin.menu.update', $product->id) }}" enctype="multipart/form-data">
                    ...
                    {{-- 🟢 NEW SECTION: Happy Hour Configuration --}}
                    <div class="mb-6 p-6 bg-amber-500/5 dark:bg-amber-900/10 rounded-xl border border-amber-200 dark:border-amber-800">
                        <div class="flex items-center gap-2 mb-4">
                            <svg class="w-5 h-5 text-amber-600" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd"></path></svg>
                            <span class="font-bold text-sm text-amber-700 dark:text-amber-400 uppercase tracking-widest">Happy Hour Protocol</span>
                        </div>
                        
                        <div class="mb-4">
                            <label class="block font-bold text-xs text-gray-600 dark:text-gray-400 mb-2 uppercase">Discount Percentage (%)</label>
                            <input type="number" name="happy_hour_discount" value="{{ old('happy_hour_discount', $product->happy_hour_discount) }}" placeholder="e.g., 20" class="w-full px-4 py-2 rounded-lg border dark:border-gray-600 bg-white dark:bg-gray-700 dark:text-white focus:ring-amber-500">
                        </div>

                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block font-bold text-xs text-gray-600 dark:text-gray-400 mb-2 uppercase">Start Time</label>
                                <input type="time" name="happy_hour_start" value="{{ old('happy_hour_start', $product->happy_hour_start) }}" class="w-full px-4 py-2 rounded-lg border dark:border-gray-600 bg-white dark:bg-gray-700 dark:text-white focus:ring-amber-500">
                            </div>
                            <div>
                                <label class="block font-bold text-xs text-gray-600 dark:text-gray-400 mb-2 uppercase">End Time</label>
                                <input type="time" name="happy_hour_end" value="{{ old('happy_hour_end', $product->happy_hour_end) }}" class="w-full px-4 py-2 rounded-lg border dark:border-gray-600 bg-white dark:bg-gray-700 dark:text-white focus:ring-amber-500">
                            </div>
                        </div>
                    </div>
