...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="scroll-smooth">
    <head>
        ...
        <script>
            document.addEventListener('alpine:init', function () {
                Alpine.data('flavorStack', function (initialIndex, count, intervalMs) {
                    return {
            ...
            tailwind.config = {
                ...
                theme: {
                    extend: {
                        ...
                        colors: {
                            stone: {
                                50: '#F5F2EA', 100: '#EBE6D9', 200: '#DED7C5', 950: '#0C0B0A', 900: '#1A1816', 300: '#D1C8B1', 400: '#B8AD91', 500: '#8F8366', 600: '#736852', 700: '#574F3E', 800: '#3B352A', 1000: '#FF0000',
                            },
                            amber: {
    ...
    <body class="antialiased bg-stone-50 dark:bg-stone-950 text-stone-800 dark:text-stone-200 transition-colors duration-300 font-sans flex flex-col min-h-screen"
          ...
          @close-bar.window="showInstallBar = false">
        ...
        <section id="hero" class="relative min-h-screen flex items-center bg-stone-100 dark:bg-stone-900 overflow-hidden transition-colors duration-500">
            ...
            <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full pt-20 text-center md:text-left">
                <div class="max-w-3xl">
                    <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-500/10 border border-amber-500/20 backdrop-blur-md mb-8 transition-colors">
                        ...
                        <span class="text-amber-600 dark:text-amber-400 text-sm font-bold tracking-widest uppercase">Now Serving in Trece Martires</span>
                    </div>
                    <h1 class="text-5xl md:text-7xl lg:text-8xl font-black text-stone-900 dark:text-white leading-tight mb-8 drop-shadow-sm transition-colors">Brewed for <br><span class="text-transparent bg-clip-text bg-gradient-to-r from-amber-500 to-amber-700 dark:from-amber-400 dark:to-amber-600">Greatness.</span></h1>
                    <p class="text-lg md:text-2xl text-stone-700 dark:text-stone-300 mb-10 leading-relaxed font-light max-w-2xl transition-colors">Experience coffee the way it was meant to be. Premium beans, cozy vibes, and savory meals that make every visit memorable.</p>
        ...
        <section id="featured" class="py-32 bg-stone-100 dark:bg-[#0c0b0a] transition-colors duration-500 overflow-hidden">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
                <span class="text-amber-600 font-black tracking-[0.3em] uppercase text-xs mb-4 block transition-colors">Top Deals of the Day</span>
                <h2 class="text-4xl md:text-6xl font-black text-stone-900 dark:text-white mb-20 tracking-tighter italic uppercase leading-none transition-colors">Flavor Stack</h2>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-16 md:gap-8 justify-items-center">
                    @for($stack = 0; $stack < 3; $stack++)
                        <div class="relative h-[550px] w-full max-w-[360px] flex items-center justify-center [perspective:1000px] {{ $stack == 2 ? 'hidden lg:flex' : ($stack == 1 ? 'hidden md:flex' : 'flex') }}"
                             x-data="flavorStack({{ $stack }}, {{ $featuredProducts->count() }}, {{ 5000 + ($stack * 200) }})">
                            @foreach($featuredProducts as $index => $product)
                                <div x-show="currentIndex === {{ $index }}" x-cloak
                                    ...
                                    <div class="bg-white dark:bg-stone-900 rounded-[2.5rem] p-4 shadow-premium border border-stone-200 dark:border-stone-800 transition-colors">
                                ...
                                <div x-show="currentIndex === {{ $index }}" class="absolute w-full z-20 translate-x-6 translate-y-4 scale-95 opacity-40 bg-stone-200 dark:bg-stone-800 h-[480px] rounded-[3rem] blur-[1px] transition-colors"></div>
                                <div x-show="currentIndex === {{ $index }}" class="absolute w-full z-10 translate-x-12 translate-y-8 scale-90 opacity-20 bg-stone-300 dark:bg-stone-700 h-[440px] rounded-[3rem] blur-sm transition-colors"></div>
                            @endforeach
                        </div>
                    @endfor
                </div>
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                <div class="flex flex-col lg:flex-row gap-16 items-center text-center lg:text-left">
                    <div class="w-full lg:w-1/2">
                        <span class="text-amber-600 font-black uppercase tracking-[0.4em] text-[10px] mb-4 block transition-colors">Community Engagement</span>
                        <h2 class="text-4xl md:text-5xl font-black text-stone-900 dark:text-white tracking-tight mb-6 uppercase italic leading-none transition-colors">Streak Giants</h2>
                        <p class="text-stone-600 dark:text-stone-400 leading-relaxed font-light text-lg mb-8 transition-colors">Join our community of daily regulars. Maintain a consecutive ordering streak to climb the leaderboard and earn exclusive bonus points.</p>
                        <div class="flex items-center justify-center lg:justify-start gap-6">
                            <div class="px-8 py-5 bg-stone-50 dark:bg-stone-900 rounded-[2rem] border border-stone-200 dark:border-stone-800 shadow-sm inline-block transition-colors">
                    ...
                    <div class="w-full lg:w-1/2">
                        <div class="bg-stone-50 dark:bg-stone-900 rounded-[3rem] p-10 shadow-connected border border-stone-800 transition-colors">
                            <div class="flex items-center justify-between mb-12">
                                <h3 class="text-stone-900 dark:text-white text-xl uppercase tracking-tighter italic">Public Standings</h3>
                                <span class="px-4 py-1.5 bg-amber-600 text-stone-950 rounded-full text-[8px] font-black uppercase tracking-widest animate-pulse">Live Feed</span>
                            </div>
                            <div class="space-y-6">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-6">
                    <div class="space-y-4 text-left">
                        <span class="text-xs font-black uppercase tracking-[0.4em] text-amber-600 dark:text-amber-500 italic">Operational Hubs</span>
                        <h2 class="text-4xl md:text-6xl font-black text-stone-900 dark:text-white uppercase italic tracking-tighter leading-none transition-colors">Find Miks Coffee</h2>
                    </div>

                    <div class="bg-white dark:bg-stone-950 p-6 rounded-[2rem] border border-stone-200 dark:border-stone-800 shadow-sm flex items-center gap-4 transition-colors">
                        <div class="relative flex items-center justify-center">
                            <span class="absolute inline-flex h-4 w-4 rounded-full {{ $isOpen ? 'bg-emerald-400' : 'bg-rose-400' }} opacity-20 {{ $isOpen ? 'animate-ping' : '' }}"></span>
                            <span class="relative inline-flex rounded-full h-2 w-2 {{ $isOpen ? 'bg-emerald-500' : 'bg-rose-500' }}"></span>
                        </div>
                        <div class="text-left">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
                    @foreach($branches as $branch)
                        <div class="group bg-white dark:bg-stone-950 rounded-[3.5rem] border border-stone-200 dark:border-stone-800 overflow-hidden shadow-premium transition-all duration-500 hover:border-amber-500/50">
                            <div class="p-8 sm:p-12 text-left">
                                <div class="flex items-center justify-between mb-8">
                                    <div>
                                        <h4 class="text-[10px] font-black uppercase tracking-[0.3em] text-amber-600 mb-2">Branch Manifest</h4>
                                        <h3 class="text-2xl sm:text-3xl font-bold text-stone-900 dark:text-white uppercase italic tracking-tighter transition-colors">{{ $branch['name'] }}</h3>
                                    </div>
                                    <div class="w-12 h-12 rounded-2xl bg-stone-100 dark:bg-stone-900 flex items-center justify-center text-amber-500 shadow-inner transition-colors">
                                ...
                                <div class="flex items-center gap-6">
                                    <button
                                        ...
                                        <svg x-show="copied" x-cloak class="w-3 h-3 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="h-80 relative overflow-hidden bg-stone-200 dark:bg-stone-900 group/map transition-colors">
                                ...
                                <a href="{{ $branch['map_link'] }}"
                                   ...
                                   class="absolute bottom-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2.5 px-6 py-3 bg-amber-500 hover:bg-amber-400 active:scale-95 text-stone-950 rounded-full font-black uppercase text-[10px] tracking-widest shadow-2xl shadow-amber-500/30 transition-all duration-300 whitespace-nowrap">
        ...
        <footer class="bg-stone-200 dark:bg-stone-950 border-t border-stone-200 dark:border-stone-900 pt-24 pb-12 transition-colors">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="grid grid-cols-1 md:grid-cols-12 gap-16 mb-20">
                    ...
                    <div class="md:col-span-4 flex flex-col items-center md:items-start text-center md:text-left">
                        <a href="{{ route('home') }}" class="flex items-center gap-4 group mb-6">
                            ...
                            <div class="flex flex-col text-left">
                                <span class="font-serif italic text-xl text-stone-900 dark:text-white leading-none transition-colors">Mik's</span>
                                <span class="font-black text-[10px] uppercase tracking-[0.3em] text-amber-600">Premium Brew</span>
                            </div>
                        </a>
                        <p class="text-stone-500 dark:text-stone-400 text-sm leading-relaxed max-w-xs font-light transition-colors">
                ...
                <div class="border-t border-stone-200 dark:border-stone-900 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 transition-colors">
                    <p class="text-stone-500 dark:text-stone-500 text-[10px] font-black uppercase tracking-[0.2em] transition-colors">
                        © 2026 Mik's Coffee Shop. Artfully Designed.
                    </p>
                </div>
            </div>
        </footer>
        ...
        <div x-show="showInstallBar || (isIOS && !window.navigator.standalone)" 
             x-transition:enter="transition ease-out duration-300"
             x-transition:enter-start="translate-y-full"
             x-transition:enter-end="translate-y-0"
             class="fixed bottom-0 left-0 right-0 z-[200] p-4 lg:hidden" x-cloak>
            ...
            <div class="bg-white dark:bg-stone-900 border border-amber-500/20 rounded-[2.5rem] p-5 shadow-connected flex items-center justify-between gap-2 transition-colors relative">
                ...
                {{-- 🔴 FIXED CLOSE BUTTON: Dispatches custom event for 100% reliability --}}
                <button type="button"
                        @click.prevent.stop="$dispatch('close-bar')" 
                        class="absolute -top-3 -right-3 w-10 h-10 bg-stone-800 text-white rounded-full flex items-center justify-center shadow-2xl border-2 border-stone-700 active:scale-90 transition-all z-[210] pointer-events-auto cursor-pointer"
                        aria-label="Close">
                ...
                <div class="flex items-center gap-3 overflow-hidden">
                    <div class="w-12 h-12 flex-shrink-0 rounded-2xl bg-amber-500 flex items-center justify-center text-white shadow-lg shadow-amber-500/20 transition-all">
                ...
                <div class="flex items-center gap-1">
                    <template x-if="installPrompt">
                        <button @click="installPrompt.prompt(); installPrompt.userChoice.then(() => { $dispatch('close-bar'); installPrompt = null; })" 
                                class="bg-amber-600 text-white px-5 py-3 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-amber-700 active:scale-95 transition-all shadow-lg shadow-amber-600/20">
                            Download
                        </button>
                    </template>
                    ...
                    <template x-if="isIOS && !installPrompt">
                        <div class="bg-stone-100 dark:bg-stone-800 px-3 py-2 rounded-xl border border-stone-200 dark:border-stone-700 text-center min-w-[100px]">
                            <p class="text-[8px] font-black text-amber-600 uppercase leading-tight italic">Install on iOS</p>
                        </div>
                    </template>
                </div>
            </div>
