<div wire:poll.5s> @if (session('success'))
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse ($orders as $order)
            <div class="bg-white dark:bg-stone-900 rounded-2xl shadow-sm border border-stone-200 dark:border-stone-800 overflow-hidden flex flex-col h-full">
                ...
                <div class="p-4 bg-stone-50 dark:bg-stone-950 border-t border-stone-100 dark:border-stone-800 mt-auto">
                    @if ($order->status === 'pending')
                        <button wire:click="updateStatus({{ $order->id }}, 'preparing')" 
                                ...
                                class="w-full bg-amber-500 hover:bg-amber-600 text-white font-bold py-3 rounded-xl transition shadow-md flex justify-center items-center gap-2">
                            <span>Start Preparing</span> <span>🔥</span>
                        </button>

                    @elseif ($order->status === 'preparing')
                        <button wire:click="updateStatus({{ $order->id }}, 'ready')" 
                                wire:loading.attr="disabled"
                                class="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 rounded-xl transition shadow-md flex justify-center items-center gap-2">
                            <span>Mark Ready</span> <span>✅</span>
                        </button>
                    @endif
                </div>
